...
(function($) {
  ...
  Private.prototype.createCacheDir = function(success_callback, error_callback) {
    ...
    const pathElements = this.imgCache.options.localCacheFolder.split("/").reverse();
    const createPathElement = (parentFolder, pathElement) => {
      parentFolder.getDirectory(pathElement, {
        ...
        const nextElement = pathElements.pop();
        if (!nextElement) {
