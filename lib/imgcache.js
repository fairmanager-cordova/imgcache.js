  ...
  this.overridables = {
    hash: function(s) {
      ...
      return (function(a) {
        var c = [],
          ...
          e;
        for (var i = 0; i < d; i++) {
          e = a[i >> 2] >> ((3 - (i % 4)) * 8);c.push(B.charAt((e >> 4) & 0xF) + B.charAt(e & 0xF))
        }
        return c.join('')
      }((function(a, b) {
        var c,
          ...
          z = 0xc3d2e1f0,
          M = [];
        U(M, 0x5a827999, 20);U(M, 0x6ed9eba1, 20);U(M, 0x8f1bbcdc, 20);U(M, 0xca62c1d6, 20);
        a[b >> 5] |= 0x80 << (24 - (b % 32));
        a[(((b + 65) >> 9) << 4) + 15] = b;
        for (var i = 0; i < h; i += 16) {
          ...
          for (var j = 0, O = []; j < 80; j++) {
            O[j] = j < 16 ? a[j + i] : L(O[j - 3] ^ O[j - 8] ^ O[j - 14] ^ O[j - 16], 1);var k = (function(a, b, c, d, e) {
    ...
    log: function(str, level) {
      ...
      if (this.options.debug) {
        if (level === ImgCacheConstants.LOG_LEVEL_INFO) {
          str = 'INFO: ' + str;
        }
        if (level === ImgCacheConstants.LOG_LEVEL_WARNING) {
          str = 'WARN: ' + str;
        }
        if (level === ImgCacheConstants.LOG_LEVEL_ERROR) {
          str = 'ERROR: ' + str;
        }
        console.log(str);
      }
...
(function($) {
  ...
  Private.prototype.createCacheDir = function(success_callback, error_callback) {
    ...
    var _getDirSuccess = function(dirEntry) {
      ...
      if (this.helpers.isCordovaAndroid() && this.imgCache.options.preventDefaultOsBehavior) {
      ...
      } else if (!this.helpers.isCordovaWindowsPhone() && this.imgCache.options.preventDefaultOsBehavior) {
    ...
    var pathElements = this.imgCache.options.localCacheFolder.split("/").reverse();
    var createPathElement = function(parentFolder, pathElement) {
      parentFolder.getDirectory(pathElement, {
        create: true,
        exclusive: false
      }, function(entry) {
        if (!entry) {
          _fail(new Error("Unable to create requested path element '" + pathElement + "'!"));
          return;
        }
        var nextElement = pathElements.pop();
        if (!nextElement) {
          _getDirSuccess(entry);
          return null;
        }
        createPathElement(entry, nextElement);
      });
    };

    createPathElement(this.imgCache.attributes.filesystem.root, pathElements.pop());
  };
  ...
  FileTransferWrapper.prototype.download = function(uri, localPath, success_callback, error_callback, on_progress) {
    ...
    if (this.fileTransfer) {
      ...
      resolverQuery.onload = function(event) {
        return this.fileTransfer.download(event.currentTarget.responseURL, localPath, success_callback, error_callback, false, {
          'headers': headers
        });
      }.bind(this);
    ...
    var dirEntry = this.imgCache.attributes.dirEntry;
  ...
  Private.prototype.loadCachedFile = function($element, img_src, set_path_callback, success_callback, error_callback) {
    ...
    this.imgCache.attributes.dirEntry.getFile(this.getCachedFileName(img_src), {
      create: false
    }, _gotFileEntry, _fail);
  };
  ...
  ImgCache.prototype.cacheFile = function(img_src, success_callback, error_callback, on_progress) {
    ...
    fileTransfer.download(
      ...
      function(entry) {
        ...
        if (entry.setMetadata && this.options.preventDefaultOsBehavior) {
          entry.setMetadata(
            function() {
              /* success*/
      ...
      function(error) {
        if (error.source) {
          this.overridables.log('Download error source: ' + error.source, ImgCacheConstants.LOG_LEVEL_ERROR);
        }
        if (error.target) {
          this.overridables.log('Download error target: ' + error.target, ImgCacheConstants.LOG_LEVEL_ERROR);
        }
  ...
  ImgCache.prototype.getCachedFile = function(img_src, response_callback) {
    ...
    var path = this.private.getCachedFileName(img_src);
    ...
    this.attributes.dirEntry.getFile(
      path,
      {
        create: false
      },
  ...
  ImgCache.prototype.removeFile = function(img_src, success_callback, error_callback) {
    ...
    var filePath = this.private.getCachedFileName(img_src);
    var _fail = function(error) {
    ...
    this.attributes.dirEntry.getFile(filePath, {
      create: false
    }, function(fileEntry) {
      fileEntry.remove(
        function() {
